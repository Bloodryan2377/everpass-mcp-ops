...
(function (global) {
  ...
  // 2026-05-23 ASK-FIX helpers ============================================
  // Resolve the Ask backend at runtime so GitHub Pages mirror falls back
  // to the Netlify host (which actually serves the function).
  function resolveAskEndpoint() {
    try {
      const host = (location && location.hostname) || '';
      if (host.endsWith('github.io')) return 'https://dashboard.everpasspipeline.com/api/ask';
    } catch (_) { /* ignore */ }
    return '/api/ask';
  }

  // Tiny safe markdown renderer for inline answers. Handles headings,
  // bold/italic, inline code, fenced code blocks, bullets, paragraphs.
  // Anything not matched is escaped.
  function renderAnswerMarkdown(target, src) {
    target.innerHTML = '';
    ...
    while (i < lines.length) {
      ...
      if (h) {
        ...
        const node = document.createElement('h' + Math.max(3, level + 2));
        node.className = 'ask-answer-h';
        node.innerHTML = inline(h[2]);
        target.appendChild(node);
        i++; continue;
      }
      if (/^\s*[-*]\s+/.test(line)) {
        const ul = document.createElement('ul');
        ul.className = 'ask-answer-ul';
      ...
      const buf = [line]; i++;
      while (i < lines.length && lines[i].trim() !== '' && !/^(#{1,4})\s+/.test(lines[i]) && !/^```/.test(lines[i]) && !/^\s*[-*]\s+/.test(lines[i])) {
        buf.push(lines[i]); i++;
      }
      const p = document.createElement('p');
      p.className = 'ask-answer-p';
      p.innerHTML = inline(buf.join(' '));
      target.appendChild(p);
    }
  }

  // Persist the answer alongside the optimistic Ask so the Answers tab
  // (which merges from localStorage) shows it on next render.
  function attachAnswerToOptimistic(correlation_id, answerText, model) {
    try {
      const raw = localStorage.getItem(OPTIMISTIC_ASKS_KEY) || '[]';
      const list = JSON.parse(raw);
      let found = false;
      for (const entry of list) {
        if (entry && entry.correlation_id === correlation_id) {
          entry.answer_snippet = String(answerText || '').slice(0, 600);
          entry.answer_text = String(answerText || '').slice(0, 8000);
          entry.answered_at = new Date().toISOString();
          entry.status = 'answered';
          entry.answer_model = model || null;
          found = true;
          break;
        }
      }
      if (found) localStorage.setItem(OPTIMISTIC_ASKS_KEY, JSON.stringify(list));
    } catch (_) { /* ignore */ }
  }
  // 2026-05-23 ASK-FIX helpers end ========================================
  ...
  function renderAskComposer() {
    ...
    // 2026-05-23 ASK-FIX: inline answer panel
    const answerPanel = el('div', {
      class: 'ask-composer-answer',
      attrs: { id: 'ask-composer-answer', 'aria-live': 'polite', hidden: 'hidden' },
    });
    card.appendChild(answerPanel);

    const ASK_ENDPOINT = resolveAskEndpoint();

    function showAnswerPanel(kind, headerText) {
      answerPanel.hidden = false;
      answerPanel.dataset.kind = kind || 'info';
      answerPanel.innerHTML = '';
      const head = el('div', { class: 'ask-answer-head' });
      head.appendChild(el('span', { class: 'ask-answer-head-label', text: headerText || '' }));
    ...
    const submitAsk = async () => {
      ...
      const askLabelOrig = btnAsk.textContent;
      btnAsk.textContent = 'Asking...';
      const panel = showAnswerPanel('pending', 'Asking Anthropic...');
      panel.appendChild(el('div', { class: 'ask-answer-spinner', text: 'thinking...' }));

      const payload = {
        question: isEmail ? (stripEmailHeaders(text) || trimmed) : trimmed,
        mode: req.mode,
        partner: '',
        correlation_id: req.correlation_id,
        source_label: 'mobile-cockpit-ask',
      };
      ...
      try {
        ...
        if (ctype.indexOf('application/json') >= 0) {
          ...
          errMsg = `non-JSON response (HTTP ${res.status}): ${raw.slice(0, 240)}`;
        }
        if (!res.ok) {
          errMsg = errMsg || (resp && (resp.error || resp.message)) || `HTTP ${res.status}`;
          resp = null;
        }
      } catch (e) {
      ...
      btnAsk.textContent = askLabelOrig;

      if (resp && resp.ok && resp.answer) {
        attachAnswerToOptimistic(req.correlation_id, resp.answer, resp.model);
        showAnswerPanel('answer', (resp.mode === 'live-brain' ? 'Live Ask Brain' : 'Standard Ask') + (resp.model ? ' · ' + resp.model : ''));
        const body = el('div', { class: 'ask-answer-body' });
        renderAnswerMarkdown(body, resp.answer);
        answerPanel.appendChild(body);
        const foot = el('div', { class: 'ask-answer-foot' });
        foot.appendChild(el('span', { class: 'ask-answer-foot-meta', text: `answered ${new Date(resp.ts || Date.now()).toLocaleTimeString()}` }));
        const copyBtn = el('button', { class: 'btn btn-ghost ask-answer-foot-btn', attrs: { type: 'button' }, text: 'Copy answer' });
        copyBtn.addEventListener('click', async () => {
          try { await navigator.clipboard.writeText(resp.answer); showToast('Answer copied.', 'success'); }
          catch (_) { showToast('Copy failed.', 'error'); }
        });
        foot.appendChild(copyBtn);
        foot.appendChild(el('a', { class: 'btn btn-ghost ask-answer-foot-btn', attrs: { href: '#/answers', role: 'button' }, text: 'Open Answers tab' }));
        answerPanel.appendChild(foot);
        showToast('Answered. Threaded into Answers tab.', 'success');
        ta.value = '';
        updateMeta();
        return;
      }

      showAnswerPanel('error', 'Ask endpoint failed');
      const errBody = el('div', { class: 'ask-answer-body' });
      errBody.appendChild(el('p', { class: 'ask-answer-p', text: errMsg || 'Unknown error.' }));
      errBody.appendChild(el('p', { class: 'ask-answer-p ask-answer-hint', text: 'Falling back to markdown-drop. Save to Files -> Ask_Drop and the desktop watcher will pick it up.' }));
      answerPanel.appendChild(errBody);
      try { downloadMarkdown(req); } catch (_) { /* ignore */ }
      showToast('Ask endpoint unreachable; markdown downloaded.', 'error');
    };
    ...
    foot.appendChild(el('span', { text: '. Tap Ask to call the cockpit Anthropic endpoint directly. Copy and Download remain available for the legacy Ask_Drop watcher path. Standard Ask = one-shot synthesis. Live Ask Brain = NotebookLM-style clarifier (1 to 3 questions) before answering.' }));
